...
# Dukascopy dosya adı varyasyonları — her TF için olası tüm formatlar
...
def find_file(sym, tf):
    ...
    # Tüm CSV ve XLS dosyalarını bul
    all_files = glob.glob(f"{s}*.csv") + glob.glob(f"{s}*.xls")

    # Bid olanları tercih et
    bid_files = [f for f in all_files if "Bid" in f or "bid" in f]
    search_files = bid_files if bid_files else all_files

    # Ask olanları çıkar
    search_files = [f for f in search_files if "Ask" not in f and "ask" not in f]

    if not search_files:
        return None

    # TF varyasyonlarından birini içereni bul
    for tf_name in tf_variants:
        matched = [f for f in search_files if tf_name in f]
        if matched:
            # Tekrar inmiş dosyalar varsa en büyüğünü al
            return max(matched, key=os.path.getsize)

    # Hiç eşleşme yoksa en büyük dosyayı al
    return max(search_files, key=os.path.getsize)

def read_xls(fp): # CSV ve XLS her ikisini de okur
    candles=[]
    try:
        ...
        for i,l in enumerate(lines):
            ll=l.lower()
            if ("time" in ll or "gmt" in ll or "date" in ll) and "open" in ll:
                start=i+1; break
