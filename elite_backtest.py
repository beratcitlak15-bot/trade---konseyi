...
def find_file(sym, tf):
    ...
    # Bu TF için tüm olası isimleri dene
    tf_variants = DUKASCOPY_TF_VARIANTS.get(tf, [DUKASCOPY_TF_MAP.get(tf, "")])

    all_candidates = []

    for t in tf_variants:
        patterns = [
            f"{s}_{t}_Bid*.csv",
            f"{s}_{t}_Bid*.xls",
            f"{s}*{t}*Bid*.csv",
        ...
        for pat in patterns:
            files = glob.glob(pat)
            bid_files = [f for f in files if "Bid" in f or "bid" in f]
            if bid_files:
                all_candidates.extend(bid_files)
            elif files:
                all_candidates.extend(files)

    if all_candidates:
        # Tekrarsız, en büyük dosyayı seç
        unique = list(set(all_candidates))
        return max(unique, key=os.path.getsize)
