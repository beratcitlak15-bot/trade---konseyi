...
# Aynı setup tekrar spam olmasın diye bekleme süresi
SIGNAL_COOLDOWN_SECONDS = 60 * 60

# Sniper giriş ayarları
MAX_BARS_AFTER_MITIGATION = 2
ENTRY_DISTANCE_MAX_MULTIPLIER = 0.50
TP_PROGRESS_BLOCK_THRESHOLD = 0.35
MIN_DISPLACEMENT_FOR_TRIGGER = ("Orta", "Güçlü")
...
def detect_order_block(
    ...
    lookback_start = max(0, len(candles) - 10)
    lookback_end = len(candles) - 1

    for i in range(lookback_end - 1, lookback_start - 1, -1):
        c = candles[i]

        if direction == "LONG" and c["close"] < c["open"]:
            future = candles[i + 1 : lookback_end + 1]
            broke = any(x["close"] > c["high"] for x in future)
            if broke and (last["close"] - c["high"]) <= avg_rng * 3.0:
                return {
                    "index": i,
                    "datetime": c["datetime"],
                    "low": c["low"],
        ...
        if direction == "SHORT" and c["close"] > c["open"]:
            ...
            if broke and (c["low"] - last["close"]) <= avg_rng * 3.0:
                return {
                    "index": i,
                    "datetime": c["datetime"],
                    "low": c["low"],
...
def validate_entry_timing(
    ...
    if entry_distance > (avg_rng * ENTRY_DISTANCE_MAX_MULTIPLIER):
        return {
            "valid": False,
            "reason": "Fiyat entry'den fazla uzaklaştı",
    ...
    if total_path > 0 and (progressed / total_path) >= TP_PROGRESS_BLOCK_THRESHOLD:
        return {
            "valid": False,
            "reason": "Fiyat TP yolunda fazla ilerledi",
            "entry_distance": entry_distance,
        }
    ...
    if touch_index is not None:
        ...
        if bars_after_touch > MAX_BARS_AFTER_MITIGATION:
            return {
                "valid": False,
                "reason": "OB touch sonrası geç kalındı",
                "entry_distance": entry_distance,
            }
...
def determine_direction(
    ...
    if (
        ...
        and displacement in MIN_DISPLACEMENT_FOR_TRIGGER
    ):
        return "LONG"
...
def build_structure_sl(
    ...
    if direction == "LONG":
        structural_anchor = min(ob["low"], sweep_low)
        return structural_anchor - buffer_val

    if direction == "SHORT":
        structural_anchor = max(ob["high"], sweep_high)
        return structural_anchor + buffer_val
...
def find_directional_target(
    ...
    if direction == "LONG":
        ...
        candidates = [x["price"] for x in swings["highs"] if x["price"] > entry]
    ...
    if direction == "SHORT":
        ...
        candidates = [x["price"] for x in swings["lows"] if x["price"] < entry]
...
def build_trade_levels(
    ...
    structure_target = find_directional_target(candles_15m, direction, entry)

    pd_levels = get_previous_day_levels(candles_5m)
    if direction == "LONG" and pd_levels["pdh"] and pd_levels["pdh"] > entry:
        if structure_target is None or pd_levels["pdh"] < structure_target:
            structure_target = pd_levels["pdh"]

    if direction == "SHORT" and pd_levels["pdl"] and pd_levels["pdl"] < entry:
        if structure_target is None or pd_levels["pdl"] > structure_target:
            structure_target = pd_levels["pdl"]

    if direction == "LONG":
        tp = max(min_rr_tp, structure_target) if structure_target else min_rr_tp
    else:
        tp = min(min_rr_tp, structure_target) if structure_target else min_rr_tp
...
def score_signal(
    ...
    if direction == "LONG":
        ...
        if mss == "Bullish MSS" or choch == "Bullish CHoCH":
            score += 8
            notes.append("Bullish structure trigger var")
    ...
    if direction == "SHORT":
        ...
        if mss == "Bearish MSS" or choch == "Bearish CHoCH":
            score += 8
            notes.append("Bearish structure trigger var")
...
# REPEAT / SPAM CONTROL
# =========================================================
def load_signal_state() -> Dict[str, Any]:
...
def save_signal_state(data: Dict[str, Any]) -> None:
    save_json_file(SIGNAL_STATE_FILE, data)
...
def build_signal_key(result: Dict[str, Any]) -> str:
    ...
    return f"{symbol}|{direction}|{entry}|{sl}"
...
def analyze_forex_symbol(
    ...
    partial_tp = (
        round_price(levels["entry"] + risk) if direction == "LONG"
    ...
    return {
        ...
        "reason": "Sniper entry zone aktif" if quality in ("A", "A+") else "Setup var ama kalite filtreyi zor geçiyor",
    }
...
def format_signal_message(r: Dict[str, Any]) -> str:
    ...
    score_notes = ", ".join(r.get("score_notes", [])[:4]) if isinstance(r.get("score_notes"), list) else "Yok"
...
def run_scan() -> int:
    ...
    for market in FOREX_METALS:
        ...
        if should_skip_repeated_signal(result, signal_state):
        ...
        if sent:
            total_signals += 1
            mark_signal_sent(result, signal_state)
