...
def find_recent_fvg_zone(
    candles: List[Dict[str, Any]],
    lookback: int = IFVG_LOOKBACK_BARS,
) -> Optional[Dict[str, Any]]:
    if len(candles) < 3:
        return None
...
def detect_ifvg_reclaim(
    ...
    if zone["type"] == "bearish":
        if last["close"] > zone["high"]:
            return {
                "valid": True,
                "direction": "LONG",
                "reason": "Bearish FVG reclaim edildi",
                "zone_low": zone["low"],
                "zone_high": zone["high"],
            }
    ...
    if zone["type"] == "bullish":
        if last["close"] < zone["low"]:
            return {
                "valid": True,
                "direction": "SHORT",
                "reason": "Bullish FVG breakdown/reclaim oldu",
                "zone_low": zone["low"],
                "zone_high": zone["high"],
            }
...
def detect_ote_retracement(
    ...
    dealing_range = get_recent_dealing_range(candles_15m, 20)
    if not dealing_range:
        return {"valid": False, "reason": "Dealing range bulunamadı"}

    zone = get_ote_zone(dealing_range["high"], dealing_range["low"], direction)
    price = candles_15m[-1]["close"]

    if zone["low"] <= price <= zone["high"]:
        return {
            "valid": True,
            "reason": "Fiyat OTE bölgesinde",
            "zone_low": zone["low"],
            "zone_high": zone["high"],
        }
    ...
    return {
        "valid": False,
        "reason": "Fiyat OTE bölgesinde değil",
        "zone_low": zone["low"],
        "zone_high": zone["high"],
    }
...
def detect_alternative_model(
    ...
    if ENABLE_IFVG_RECLAIM:
        ...
        if ifvg["valid"]:
            ...
            if direction == "LONG" and bias == "Düşüş":
                pass
            elif direction == "SHORT" and bias == "Yükseliş":
                pass
            else:
                return {
                    "valid": True,
    ...
    if ENABLE_OTE_RETRACEMENT:
        if bias in ("Yükseliş", "Düşüş"):
